...
Authentication is handled at **connection time** via a JWT passed as a query parameter. The token must belong to a user with role `customer` or `admin`.
...
- The user's role is not `customer` or `admin`
